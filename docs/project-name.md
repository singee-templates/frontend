...
- Regenerate local IDE metadata instead of preserving template-specific `.idea`
  files. Some IDE files can contain the old directory name `frontend`, but they
...
```bash
rg -n \
  "@singee-templates/frontend|singee-templates/frontend|Frontend Template|TanStack Start Starter|TanStack App|Create TanStack App Sample|your-username|your-repo|my-app" \
  -S \
